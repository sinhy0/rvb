...
apkmirror_search() {
	...
	# 1) arm64-v8a + DPI >= 522 -> smallest file.
	# 2) if none: arm64-v8a/universal/noarch + nodpi -> smallest file.
	# 3) if none: arm64-v8a/universal/noarch -> closest DPI to 522.
	#    Size is ignored in stage 3.
	# APK and Bundle are treated equally.
	...
	_variant_stage() {
		...
		# Stage 1: arm64-v8a and DPI >= 522.
		if [ "$a" = "arm64-v8a" ]; then
			if [[ "$d" =~ ^([0-9]+)$ ]]; then
				(( ${BASH_REMATCH[1]} >= target_dpi )) && { echo 1; return; }
			elif [[ "$d" =~ ^([0-9]+)-([0-9]+)$ ]]; then
				hi="${BASH_REMATCH[2]}"
				(( hi >= target_dpi )) && { echo 1; return; }
			fi
		fi

		# Stage 2: accepted architectures + nodpi only.
		[ "$d" = "nodpi" ] && { echo 2; return; }

		# Stage 3: accepted architectures.
		echo 3
	}

	# Read the exact byte count from the same variant row when present.
	# Example: 124.31 MB (130,350,399 bytes)
	_variant_size_bytes() {
		local row="$1" bytes=""
	...
	# Preserve APKMirror's original row discovery mechanism.
	for ((n = 1; n < 80; n++)); do
		...
		local node_type node_arch node_dpi stage distance size
		node_type=$($HTMLQ \
		...
		_variant_arch_ok "$node_arch" || continue

		stage=$(_variant_stage "$node_arch" "$node_dpi")
		distance=$(_variant_dpi_distance "$node_dpi")

		# Unknown DPI cannot participate in stage 3.
		if [ -z "$distance" ]; then
			continue
		fi

		size=$(_variant_size_bytes "$node")
	...
	for c in "${candidates[@]}"; do
		...
		if [ "$best_stage" -eq 1 ] || [ "$best_stage" -eq 2 ]; then
			# Stage 1/2: smallest file size only.
			if [ "$sz" -lt "$best_size" ]; then
				best_url="$u"; best_type="$t"; best_arch="$a"; best_dpi="$d"
				best_size="$sz"; best_distance="$dist"; found=true
			fi
		else
			# Stage 3: closest DPI only. Size is intentionally ignored.
