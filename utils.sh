...
PY
	...
	if [ "$is_bundle" = true ]; then
		rm -f "$output.apkm"
		if req "$direct_url" "$output.apkm" && _uptodown_valid_archive "$output.apkm"; then
			merge_splits "${output%.apk}.apkm" "${output}"
		else
			local api_direct
			...
			rm -f "$output.apkm"
			req "$api_direct" "$output.apkm" || return 1
			_uptodown_valid_archive "$output.apkm" || return 1
			merge_splits "${output%.apk}.apkm" "${output}"
		fi
	else
		rm -f "$output"
		if req "$direct_url" "$output" && _uptodown_valid_archive "$output"; then
			return 0
		fi
		local api_direct
		api_direct=$(_uptodown_api_direct_url "$best_id" 2>/dev/null || true)
		...
		rm -f "$output"
		req "$api_direct" "$output" || return 1
