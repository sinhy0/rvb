...
dl_uptodown() {
	...
	local data_version files node_arch="" node_class data_file_id
	...
	if [ -n "$data_version" ]; then
		...
		local candidates=()
		local max_children=160
		for ((n = 1; n <= max_children; n++)); do
			node_class=$($HTMLQ -w -t ".content > :nth-child($n)" --attribute class <<<"$files" 2>/dev/null || true)
			[ -z "$node_class" ] && break
			if [ "$node_class" != "variant" ]; then
				node_arch=$($HTMLQ -w -t ".content > :nth-child($n)" <<<"$files" 2>/dev/null | xargs || true)
				continue
			fi
			[ -z "$node_arch" ] && continue

			local variant_html variant_text node_dpi file_type size_bytes sd stage distance
			variant_html=$($HTMLQ -w -t ".content > :nth-child($n)" <<<"$files" 2>/dev/null || true)
			variant_text=$($HTMLQ -w -t ".content > :nth-child($n)" <<<"$files" 2>/dev/null | tr '\n' ' ' | xargs || true)
			...
			file_type="${file_type,,}"
			[ "$file_type" = "xapk" ] && local_candidate_bundle=true || local_candidate_bundle=false
