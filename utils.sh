...
apkmirror_search() {
	...
	#   1) arm64-v8a with a DPI/range that reaches 522 or higher -> smallest file
	#   2) if none: arm64-v8a/universal/noarch + nodpi/anydpi -> smallest file
	#   3) if none: arm64-v8a/universal/noarch -> closest DPI to 522; size breaks ties
	# APK and BUNDLE are considered together.

	dpi_kind() {
		local value="${1,,}"
		value="$(xargs <<<"$value")"
		value="${value//dpi/}"
		if [[ "$value" == "nodpi" || "$value" == "anydpi" || "$value" == "any" ]]; then
			echo "special"
			return 0
		fi
	...
	arch_allowed() {
		...
		a="${a//[[:space:]]/}"
		case "$a" in
			arm64-v8a|universal|noarch|all) return 0 ;;
			*) return 1 ;;
		esac
	}

	# Extract the actual file size from the APKMirror variant row.
	# This is intentionally read from the row itself, not from the download
	# redirect page, so one variant cannot accidentally inherit another size.
	row_size_bytes() {
		local row="$1" text="" bytes="" s=""
		text=$($HTMLQ "div.table-cell" --text <<<"$row" 2>/dev/null |
			tr '
' ' ' | tr -s ' ') || true

		# APKMirror release rows may expose the exact byte count as:
		#   124.31 MB (130,350,399 bytes)
		# Prefer that exact byte count when it is present.
		...
		# Safe fallback: preserve the working selector behaviour if the
		# row does not contain an explicit "(N bytes)" field.
	...
	for ((n = 1; n < 80; n++)); do
		...
		if [ "$kind" = "special" ]; then
			...
			if (( lo >= target_dpi )); then
				[ "$node_arch" = "arm64-v8a" ] && stage=1
				distance=$((lo-target_dpi))
			else
				distance=$((target_dpi-lo))
			fi
		elif [[ "$kind" == range:* ]]; then
			IFS=: read -r _ lo hi <<<"$kind"
			# A range qualifies for stage 1 when its supported DPI range
			# reaches 522 or above.
			if (( hi >= target_dpi )); then
				[ "$node_arch" = "arm64-v8a" ] && stage=1
				if (( lo >= target_dpi )); then
					distance=$((lo-target_dpi))
				else
		...
		# Keep the row's variant identity and its measured size together.
	...
	local best_size=999999999999999 best_distance=999999
	local found=false
