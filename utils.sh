...
apkmirror_search() {
	...
	for ((n = 1; n < 80; n++)); do
		...
		if [ "$kind" = "special" ]; then
			...
			# Preferred variant: arm64-v8a with DPI >= 522.
			# For DPI ranges, the range qualifies when its lower bound is >= 522.
			if [ "$node_arch" = "arm64-v8a" ] && (( lo >= target_dpi )); then
				stage=1
				distance=0
	...
	# Determine the winning stage first. Stage 1 is only arm64 + DPI containing 522.
	...
	# Stage 3 must be limited to the allowed architectures; already enforced above.
	# For stages 1 and 2, smallest file wins. For stage 3, closest DPI wins.
