...
dl_uptodown() {
	...
	if [ -n "$data_version" ]; then
		...
		# IMPORTANT: data-file-id is not consistently the URL token used by
		# Uptodown's current download flow.  Variant rows commonly expose a
		# /android/post-download/<id> target.  Try that first, then the older
		# /download/<id> and /download/<id>-x endpoints as compatibility fallbacks.
		# This is also why constructing https://dw.uptodown.com/dwn/ directly from
		# the variant ID is wrong: the CDN token is generated by the post-download
		# page and can be completely different from data-file-id.
		__UPTODOWN_DIRECT_URL__=""
		local selected_data_url="" selected_path resp_try
		local selected_paths=("/post-download/${best_id}" "/download/${best_id}" "/download/${best_id}-x")
		for selected_path in "${selected_paths[@]}"; do
			...
			if [ -z "$selected_data_url" ]; then
				selected_data_url=$(grep -oE 'data-url="[^"]+"' <<<"$resp_try" | head -1 | sed 's/^data-url="//;s/"$//' || true)
			fi
			if [ -z "$selected_data_url" ]; then
