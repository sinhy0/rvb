...
dl_uptodown() {
	...
	if [ -n "$data_version" ]; then
		...
		# Uptodown variant policy is intentionally different from APKMirror.
		# Do NOT apply the global DPI/nodpi/universal rules here. For Uptodown,
		# select an arm64-v8a variant only. If several arm64-v8a variants exist,
		# choose the smallest file; page order is never used.
		...
		for ((n = 1; n <= max_children; n++)); do
			...
			if ! variant_html=$($HTMLQ -w ".content > :nth-child($n).variant" <<<"$files" 2>/dev/null); then
				variant_html=""
			fi
			...
			local variant_text node_dpi file_type size_bytes data_file_id normalized_arch
			variant_text=$(printf '%s' "$variant_html" | tr '\n' ' ' | xargs || true)
			normalized_arch=$(tr '[:upper:]' '[:lower:]' <<<"$node_arch" | xargs)

			# Only the pure arm64-v8a architecture is accepted. Combined/fat
			# architectures, universal, noarch and armeabi-v7a are intentionally
			# excluded for Uptodown. DPI is deliberately ignored.
			[ "$normalized_arch" = "arm64-v8a" ] || continue

			node_dpi=$($HTMLQ ".content > :nth-child($n)" --attribute data-dpi <<<"$files" 2>/dev/null || true)
			[ -z "$node_dpi" ] && node_dpi=$(grep -oE '(nodpi|[0-9]+(-[0-9]+)?dpi)' <<<"$variant_text" | head -1 || true)
			...
			local post_download_id post_download_path
			# Uptodown embeds the variant-specific post-download path in the
			# variant HTML. It is NOT guaranteed to be numeric; keep the complete
			# path component instead of assuming it matches the file ID.
			post_download_path=$(grep -oE 'android/post-download/[^\"\x27[:space:]<>]+' <<<"$variant_html" | head -1 || true)
			post_download_path=${post_download_path#android/post-download/}
			post_download_id="$post_download_path"
			candidates+=("${data_file_id}|${file_type}|${node_arch}|${node_dpi}|${size_bytes}|${post_download_id}")
		done

		[ ${#candidates[@]} -gt 0 ] || return 1

		# Pick the smallest arm64-v8a file. This is the only Uptodown
		# architecture/variant selection rule.
		local best_id="" best_type="" best_size=999999999999999 best_post_id="" found=false c
		for c in "${candidates[@]}"; do
			local cid ct ca cd csz cpost
			IFS='|' read -r cid ct ca cd csz cpost <<<"$c"
			if [ "$csz" -lt "$best_size" ]; then
				best_id="$cid"; best_type="$ct"; best_size="$csz"; best_post_id="$cpost"; found=true
			fi
		done
		[ "$found" = true ] || return 1
		is_bundle=false
		...
		pr "Uptodown variant selected: id=${best_id}, arch=arm64-v8a, size=${best_size} bytes"
		local selected_download_url="${uptodown_dlurl}/download/${best_id}-x"
		local selected_data_url="" download_page="" post_page=""

		# IMPORTANT: Uptodown's current flow exposes the real CDN token through
		# the variant-specific post-download endpoint. Do not manufacture a token
		# from the file id. The variant HTML contains android/post-download/<id>.
		if [ -n "$best_post_id" ]; then
			local post_url="${uptodown_dlurl}/post-download/${best_post_id}"
			pr "Uptodown post-download URL: ${post_url}"
			post_page=$(req "$post_url" - 2>/dev/null || true)
			selected_data_url=$(sed -n 's;.*class="post-download"[^>]*data-url="\([^"]*\)".*;\1;p' <<<"$post_page" | head -1 || true)
			# Be tolerant if Uptodown changes the attribute order.
			[ -z "$selected_data_url" ] && selected_data_url=$(sed -n 's;.*data-url="\([^"]*\)"[^>]*class="post-download".*;\1;p' <<<"$post_page" | head -1 || true)
			[ -z "$selected_data_url" ] && selected_data_url=$($HTMLQ '.post-download' --attribute data-url <<<"$post_page" 2>/dev/null || true)
		fi

		# Canonical fallback: open the selected variant's download page and read
		# the download button token. This is also useful if Uptodown changes the
		# post-download markup.
		if [ -z "$selected_data_url" ]; then
			pr "Uptodown variant download page: ${selected_download_url}"
			download_page=$(req "$selected_download_url" - 2>/dev/null || true)
			selected_data_url=$($HTMLQ '#detail-download-button' --attribute data-url <<<"$download_page" 2>/dev/null || true)
		fi

		# Last web fallback: Cloudflare/Trawl, same exact variant URL.
		...
		if [ -n "$selected_data_url" ] && [ "$selected_data_url" != "apps" ]; then
			if [[ "$selected_data_url" == https://* ]]; then
				direct_url="$selected_data_url"
			elif [[ "$selected_data_url" == */* ]]; then
				direct_url="https://dw.uptodown.com/dwn/${selected_data_url}"
			else
				direct_url="https://dw.uptodown.com/dwn/${selected_data_url}"
			fi
			pr "Uptodown CDN URL: ${direct_url}"
		else
			pr "ERROR: Uptodown did not expose a valid data-url for variant ${best_id}"
			return 1
		fi
	else
		return 1
	...
	local data_url
	direct_url="${direct_url:-${__UPTODOWN_DIRECT_URL__:-}}"
