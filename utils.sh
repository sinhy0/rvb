...
dl_uptodown() {
	...
	if [ -n "$data_version" ]; then
		...
		# Uptodown's web flow is: variant -> /download/<file_id>-x ->
		# #detail-download-button[data-url] -> dw.uptodown.com/dwn/<token>.
		# Keep this isolated to the Uptodown downloader; other sources are untouched.
		__UPTODOWN_DIRECT_URL__=""
		local selected_download_url="${uptodown_dlurl}/download/${best_id}-x"
		local download_page="" selected_data_url=""

		# First use the same canonical flow used by current ReVanced/Morphe
		# Uptodown integrations. Prefer the normal req() path because it preserves
		# the project's existing cookies/User-Agent handling.
		if download_page=$(req "$selected_download_url" - 2>/dev/null); then
			selected_data_url=$($HTMLQ '#detail-download-button' --attribute data-url <<<"$download_page" 2>/dev/null || true)
		fi

		# Some Uptodown pages expose the redirect as an android/post-download URL
		# instead of putting the final token directly on the button.
		if [ -z "$selected_data_url" ] && [ -n "$download_page" ]; then
			local post_id=""
			post_id=$(sed -n 's;.*android/post-download/\([0-9][0-9]*\).*;\1;p' <<<"$download_page" | head -1 || true)
			# Current Uptodown pages can expose the post-download endpoint without
			# embedding its ID in the static HTML. The selected file ID is the
			# correct fallback for that case.
			[ -n "$post_id" ] || post_id="$best_id"
			if [ -n "$post_id" ]; then
				local post_page=""
				post_page=$(req "${uptodown_dlurl}/post-download/${post_id}" - 2>/dev/null || true)
				selected_data_url=$(sed -n 's;.*class="post-download"[^>]*data-url="\([^"]*\)".*;\1;p' <<<"$post_page" | head -1 || true)
				[ -z "$selected_data_url" ] && selected_data_url=$($HTMLQ '.post-download' --attribute data-url <<<"$post_page" 2>/dev/null || true)
			fi
		fi

		# If the normal HTTP client was blocked, try the project's Cloudflare/Trawl
		# fetcher for the same exact page, then repeat the extraction.
		if [ -z "$selected_data_url" ] && _cf_get "$selected_download_url"; then
			download_page="$html"
			selected_data_url=$($HTMLQ '#detail-download-button' --attribute data-url <<<"$download_page" 2>/dev/null || true)
		...
		# Accept only a real Uptodown token or a full CDN URL. Never accept generic
		# values such as data-url="apps".
		if [ -n "$selected_data_url" ] && [ "$selected_data_url" != "apps" ]; then
			if [[ "$selected_data_url" == https://dw*.uptodown.*/* ]]; then
				__UPTODOWN_DIRECT_URL__="$selected_data_url"
			else
				__UPTODOWN_DIRECT_URL__="https://dw.uptodown.com/dwn/${selected_data_url}"
			fi
		fi

		[ -n "$__UPTODOWN_DIRECT_URL__" ] || return 1
	fi
	...
	if [ -z "$direct_url" ]; then
		...
		if [[ "$data_url" == https://* ]]; then
			...
			direct_url="https://dw.uptodown.com/dwn/${data_url}"
		fi
	fi
...
PY
	...
	if [ "$is_bundle" = true ]; then
		...
		# Uptodown currently serves some signed files from dw.uptodown.net even
		# when the page token is constructed against dw.uptodown.com. Retry the
		# exact same signed path on .net before falling back to eAPI.
		if [[ "$direct_url" == https://dw.uptodown.com/* ]]; then
			local net_direct="${direct_url/https:\/\/dw.uptodown.com/https:\/\/dw.uptodown.net}"
			rm -f "$output"
			if _uptodown_curl_download "$net_direct" "$output" "$selected_download_url" >/dev/null 2>&1 && _uptodown_valid_archive "$output"; then
				return 0
			fi
		fi
		local api_direct
