...
apkmirror_search() {
	...
	# apk_bundle may be APK, BUNDLE, or ANY. ANY is used by automatic
	# selection so APK and BUNDLE variants compete under the same DPI rules.
	local selection_mode="${apk_bundle^^}"
	...
	local specific_url="" specific_score=999999 specific_type=""
	local specific_fallback_url="" specific_fallback_score=999999
	local universal_url="" universal_score=999999 universal_type=""
	...
	for ((n = 1; n < 40; n++)); do
		...
		# In automatic mode do NOT filter APK/BUNDLE before scoring.
		# The architecture/DPI rules decide the winner across both types.
		if [ "$selection_mode" != "ANY" ] && [ "$node_apk_bundle" != "$apk_bundle" ]; then
			continue
		fi
		...
		if is_universal_arch "$node_arch"; then
			if [ "$score" -lt "$universal_score" ]; then
				...
				universal_url="$dlurl"
				universal_type="$node_apk_bundle"
			fi
		fi
	done
	...
	if [ -n "$specific_url" ]; then
		if [ "$selection_mode" = "ANY" ]; then
			echo "${specific_type}|${specific_url}"
		else
			echo "$specific_url"
		fi
		return 0
	...
	if [ -n "$universal_url" ]; then
		if [ "$selection_mode" = "ANY" ]; then
			echo "${universal_type}|${universal_url}"
		else
			echo "$universal_url"
		fi
		return 0
...
dl_apkmirror() {
	...
	if [ "$node" ]; then
		...
		if [ "${args[prefer_dl_mode]:-}" = "bundle" ]; then
			# Explicit bundle preference is preserved: try BUNDLE first, then APK.
			if selection=$(apkmirror_search "$resp" "$dpi" "$arch" "BUNDLE" "$clean_search_version" "$search_version"); then
				selected_type="BUNDLE"
				dlurl="$selection"
			else
				if ! dlurl=$(apkmirror_search "$resp" "$dpi" "$arch" "APK" "$clean_search_version" "$search_version"); then
					return 1
				fi
				selected_type="APK"
			fi
		else
			# Automatic mode: compare APK and BUNDLE together. The winning
			# architecture/DPI score, not file type or row order, decides.
			selection=$(apkmirror_search "$resp" "$dpi" "$arch" "ANY" "$clean_search_version" "$search_version") || return 1
			selected_type="${selection%%|*}"
			dlurl="${selection#*|}"
		fi

		[ "$selected_type" = "BUNDLE" ] && is_bundle=true || is_bundle=false
		[ -z "$dlurl" ] && return 1
