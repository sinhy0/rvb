...
source_release_pick_from_list() {
	...
	case "$host" in
		github)
			# Global policy: stable/dev/beta/alpha/rc are all eligible.
			# Pick strictly by real publication time, with created_at fallback.
			jq -e -c 'map(select(.tag_name != null and .tag_name != "")) | sort_by(.published_at // .created_at // "") | reverse | .[0] // empty'
			;;
		gitlab)
			# Global policy: release channel is irrelevant.
...
apkmirror_search() {
	local resp="$1" dpi="$2" arch="$3" _apk_bundle="$4" clean_search_version="$5" search_version="$6"
	local target_dpi=522 node dlurl

	# GLOBAL APKMirror rule:
	# 1) arm64-v8a + density >= 522 -> smallest exact byte size.
	# 2) if none: arm64-v8a/universal/noarch + nodpi -> smallest exact byte size.
	# 3) if none: arm64-v8a/universal/noarch -> closest DPI to 522.
	#    Size is NOT a tie-breaker in stage 3.
	# APK and Bundled APK are equal.

	variant_size_bytes() {
		local variant_url="$1" page="" plain="" exact="" size_text=""
		__SILENT_CF_GET__=true _cf_get "$variant_url" || true
		page="$html"
		[ -z "$page" ] && { echo 999999999999999; return; }
		plain=$(sed -E 's/<[^>]+>/ /g' <<<"$page" | tr '\r\n' ' ' | tr -s ' ') || true
		exact=$(grep -oP '\([[:space:]]*[0-9][0-9,]*[[:space:]]+bytes[[:space:]]*\)' <<<"$plain" | head -1 | grep -oP '[0-9][0-9,]*' | tr -d ',') || true
		if [ -n "$exact" ]; then echo "$exact"; return; fi
		size_text=$(grep -oP 'File size:[[:space:]]*[0-9]+([.][0-9]+)?[[:space:]]*(KB|MB|GB)' <<<"$plain" | head -1) || true
		[ -z "$size_text" ] && size_text=$(grep -oP '[0-9]+([.][0-9]+)?[[:space:]]*(KB|MB|GB)' <<<"$plain" | head -1) || true
		size_bytes "$size_text"
	}

	# Returns: stage|distance. Architecture matching is intentionally
	# substring-based for arm64-v8a so APKMirror variants such as
	# "arm64-v8a + armeabi-v7a" are treated as ARM64 candidates.
	# For a DPI range, Stage 1 applies when the range reaches/includes 522.
	variant_stage_distance() {
		local a="${1,,}" d="${2,,}" raw lo hi value
		a="$(xargs <<<"$a")"; d="$(xargs <<<"$d")"; raw="$d"

		# Allowed architectures: ARM64 (including combined/fat variants),
		# universal, and noarch. Other architectures are never candidates.
		local is_arm64=false
		[[ "$a" == *arm64-v8a* ]] && is_arm64=true
		if [ "$is_arm64" = false ] && [ "$a" != "universal" ] && [ "$a" != "noarch" ]; then
			echo ""
			return
		fi

		if [ "$raw" = "nodpi" ]; then echo "2|0"; return; fi
		# Strip only a trailing dpi suffix; do not alter the "dpi" inside nodpi.
		raw="${raw%dpi}"

		if [[ "$raw" =~ ^([0-9]+)-([0-9]+)$ ]]; then
			lo="${BASH_REMATCH[1]}"; hi="${BASH_REMATCH[2]}"
			# Stage 1: ARM64 whose DPI range reaches/includes target DPI.
			if [ "$is_arm64" = true ] && (( hi >= target_dpi )); then
				echo "1|0"
				return
			fi
			if (( target_dpi < lo )); then echo "3|$((lo-target_dpi))"
			elif (( target_dpi > hi )); then echo "3|$((target_dpi-hi))"
			else echo "3|0"; fi
			return
		fi
		...
		if [[ "$raw" =~ ^([0-9]+)$ ]]; then
			...
			if [ "$is_arm64" = true ] && (( value >= target_dpi )); then
				echo "1|$((value-target_dpi))"
			else
				if (( value >= target_dpi )); then echo "3|$((value-target_dpi))"; else echo "3|$((target_dpi-value))"; fi
			fi
			return
		fi
		echo ""
	...
	local candidates=()
	for ((n=1; n<80; n++)); do
		node=$($HTMLQ "div.table-row.headerFont:nth-last-child($n)" <<<"$resp" 2>/dev/null || true)
		[ -z "$node" ] && break
		dlurl=$($HTMLQ --base https://www.apkmirror.com --attribute href \
			"div.table-cell:nth-child(1) > a:nth-child(1)" <<<"$node" 2>/dev/null || true)
		[ -z "$dlurl" ] && continue

		local node_type node_arch node_dpi sd stage distance size
		node_type=$($HTMLQ "div.table-cell:nth-child(1) span.apkm-badge:first-of-type" --text <<<"$node" 2>/dev/null | xargs || true)
		[ -z "$node_type" ] && node_type="APK"
		node_arch=$($HTMLQ "div.table-cell:nth-child(2)" --text <<<"$node" 2>/dev/null | xargs || true)
		node_dpi=$($HTMLQ "div.table-cell:nth-child(4)" --text <<<"$node" 2>/dev/null | xargs || true)

		if [ -n "$clean_search_version" ] && [[ "$dlurl" != *"$clean_search_version"* ]] && [[ "$dlurl" != *"$search_version"* ]]; then continue; fi
		sd=$(variant_stage_distance "$node_arch" "$node_dpi")
		[ -n "$sd" ] || continue
		stage="${sd%%|*}"; distance="${sd#*|}"
		size=$(variant_size_bytes "$dlurl")
		candidates+=("${dlurl}|${node_type}|${node_arch}|${node_dpi}|${stage}|${distance}|${size}")
	done

	[ ${#candidates[@]} -gt 0 ] || return 1
	local best_stage=99 c
	for c in "${candidates[@]}"; do
		local st; st=$(cut -d'|' -f5 <<<"$c"); [ "$st" -lt "$best_stage" ] && best_stage="$st"
	done

	local best_url="" best_type="" best_size=999999999999999 best_distance=999999 found=false
	for c in "${candidates[@]}"; do
		local u t a d st dist sz
		IFS='|' read -r u t a d st dist sz <<<"$c"
		[ "$st" -eq "$best_stage" ] || continue
		if [ "$best_stage" -eq 1 ] || [ "$best_stage" -eq 2 ]; then
			if [ "$sz" -lt "$best_size" ]; then best_url="$u"; best_type="$t"; best_size="$sz"; found=true; fi
		else
				# Stage 3: DPI proximity decides. Size is only a tie-breaker
			# when two candidates have the same DPI distance.
			if [ "$dist" -lt "$best_distance" ] || { [ "$dist" -eq "$best_distance" ] && [ "$sz" -lt "$best_size" ]; }; then
				best_url="$u"; best_type="$t"; best_distance="$dist"; best_size="$sz"; found=true
			fi
		fi
	done
	[ "$found" = true ] || return 1
	pr "APKMirror variant selected: arch=$(cut -d'|' -f3 <<<"$(printf '%s\n' "${candidates[@]}" | grep -F "$best_url" | head -1)") , dpi=$(cut -d'|' -f4 <<<"$(printf '%s\n' "${candidates[@]}" | grep -F "$best_url" | head -1)"), stage=${best_stage}, size=${best_size} bytes"
	echo "${best_type}|${best_url}"
	return 0
}
dl_apkmirror() {
	...
	local node dlurl="" selected=""
	node=$($HTMLQ "div.table-row.headerFont:nth-last-child(1)" -r "span:nth-child(n+3)" <<<"$resp")
	if [ "$node" ]; then
		selected=$(apkmirror_search "$resp" "$dpi" "$arch" "" "$clean_search_version" "$search_version") || return 1
		is_bundle=false
...
get_uptodown_resp() {
	...
	# Uptodown may return 404 for /versions to non-browser clients even though
	# the normal app page and its version API are available. The app page is
	# sufficient for data-code and download metadata, so use it as a fallback.
	if ! __UPTODOWN_RESP__=$(req "${url}/versions" - 2>/dev/null); then
		pr "Uptodown /versions unavailable; falling back to app page"
		__UPTODOWN_RESP__=$(req "${url}" -) || return 1
	fi
	if ! __UPTODOWN_RESP_PKG__=$(req "${url}/download" - 2>/dev/null); then
		__UPTODOWN_RESP_PKG__="$__UPTODOWN_RESP__"
	fi
...
dl_uptodown() {
	...
	# Uptodown's variants endpoint exposes architecture groups followed by one
	# or more .variant rows.  Apply the same global selection policy used by
	# APKMirror instead of relying on page order or preferring fat bundles.
	local data_version files node_arch="" data_file_id
	data_version=$($HTMLQ '.button.variants' --attribute data-version <<<"$resp" 2>/dev/null || true)
	if [ -n "$data_version" ]; then
		files=$(req "${uptodown_dlurl%/*}/app/${data_code}/version/${data_version}/files" - | jq -e -r .content) || return 1

		# Return: stage|distance. Empty means the architecture is not allowed.
		_uptodown_variant_stage_distance() {
			local a="${1,,}" d="${2,,}" raw lo hi value target_dpi=522
			a="$(xargs <<<"$a")"; d="$(xargs <<<"$d")"; raw="$d"
			local is_arm64=false
			[[ "$a" == *arm64-v8a* ]] && is_arm64=true
			if [ "$is_arm64" = false ] && [ "$a" != "universal" ] && [ "$a" != "noarch" ]; then
			...
			if [[ "$raw" =~ ^([0-9]+)-([0-9]+)$ ]]; then
				lo="${BASH_REMATCH[1]}"; hi="${BASH_REMATCH[2]}"
				if [ "$is_arm64" = true ] && (( hi >= target_dpi )); then echo "1|0"; return; fi
				if (( target_dpi < lo )); then echo "3|$((lo-target_dpi))"
				elif (( target_dpi > hi )); then echo "3|$((target_dpi-hi))"
				else echo "3|0"; fi
				return
		...
		_uptodown_size_bytes() {
			local text="${1:-}" exact num unit
			exact=$(grep -oE '[0-9][0-9,]*[[:space:]]+bytes' <<<"$text" | head -1 | grep -oE '[0-9][0-9,]*' | tr -d ',') || true
			if [ -n "$exact" ]; then echo "$exact"; return; fi
			if [[ "$text" =~ ([0-9]+([.][0-9]+)?)[[:space:]]*(GB|MB|KB) ]]; then
				num="${BASH_REMATCH[1]}"; unit="${BASH_REMATCH[3]}"
				awk -v n="$num" -v u="$unit" 'BEGIN { m=(u=="GB"?1024*1024*1024:(u=="MB"?1024*1024:1024)); printf "%.0f", n*m }'
				return
			fi
		...
		local candidates=()
		local max_children=160 child_html
		for ((n = 1; n <= max_children; n++)); do
			# Do not inspect the generic `class` attribute here: htmlq can return
			# a non-zero status when an element has no class attribute, which
			# would make the parser stop before reaching the first variant.
			child_html=$($HTMLQ -w -t ".content > :nth-child($n)" <<<"$files" 2>/dev/null || true)
			[ -z "$child_html" ] && continue
			if ! variant_html=$($HTMLQ -w -t ".content > :nth-child($n).variant" <<<"$files" 2>/dev/null); then
				variant_html=""
			fi
			if [ -z "$variant_html" ]; then
				node_arch=$(xargs <<<"$child_html" || true)
				continue
			fi
			[ -z "$node_arch" ] && continue

			local variant_text node_dpi file_type size_bytes sd stage distance
			variant_text=$(printf '%s' "$variant_html" | tr '\n' ' ' | xargs || true)

			# Prefer an explicit DPI attribute if Uptodown provides one; otherwise
			# parse the visible variant text (e.g. 120-640dpi / nodpi).
			node_dpi=$($HTMLQ ".content > :nth-child($n)" --attribute data-dpi <<<"$files" 2>/dev/null || true)
			if [ -z "$node_dpi" ]; then
				node_dpi=$(grep -oE '(nodpi|[0-9]+(-[0-9]+)?dpi)' <<<"$variant_text" | head -1 || true)
			fi
			[ -z "$node_dpi" ] && node_dpi="nodpi"

			file_type=$($HTMLQ -w -t ".content > :nth-child($n) > .v-file > span" <<<"$files" 2>/dev/null || true)
			[ -z "$file_type" ] && file_type=$(grep -oEi '\b(xapk|apk)\b' <<<"$variant_text" | head -1 || true)
			[ -z "$file_type" ] && file_type="apk"
			file_type="${file_type,,}"
			# Keep exact bytes when present in the HTML; otherwise use the displayed
			# file size. This is sufficient for normal Uptodown rows and, critically,
			# makes size the tie-breaker rather than the row order.
			size_bytes=$(_uptodown_size_bytes "$variant_html $variant_text")
			sd=$(_uptodown_variant_stage_distance "$node_arch" "$node_dpi")
			[ -n "$sd" ] || continue
			stage="${sd%%|*}"; distance="${sd#*|}"
			data_file_id=$($HTMLQ ".content > :nth-child($n) > .v-report" --attribute data-file-id <<<"$files" 2>/dev/null || true)
			[ -n "$data_file_id" ] || continue
			candidates+=("${data_file_id}|${file_type}|${node_arch}|${node_dpi}|${stage}|${distance}|${size_bytes}")
		done

		[ ${#candidates[@]} -gt 0 ] || return 1
		local best_stage=99 c
		for c in "${candidates[@]}"; do
			local st; st=$(cut -d'|' -f5 <<<"$c")
			[ "$st" -lt "$best_stage" ] && best_stage="$st"
		done

		local best_id="" best_type="" best_size=999999999999999 best_distance=999999 found=false
		for c in "${candidates[@]}"; do
			local cid ct ca cd cst cdist csz
			IFS='|' read -r cid ct ca cd cst cdist csz <<<"$c"
			[ "$cst" -eq "$best_stage" ] || continue
			if [ "$best_stage" -eq 1 ] || [ "$best_stage" -eq 2 ]; then
				if [ "$csz" -lt "$best_size" ]; then
					best_id="$cid"; best_type="$ct"; best_size="$csz"; found=true
				fi
			else
				if [ "$cdist" -lt "$best_distance" ] || { [ "$cdist" -eq "$best_distance" ] && [ "$csz" -lt "$best_size" ]; }; then
					best_id="$cid"; best_type="$ct"; best_distance="$cdist"; best_size="$csz"; found=true
				fi
			fi
		done
		[ "$found" = true ] || return 1
		is_bundle=false
		[ "$best_type" = "xapk" ] && is_bundle=true
		pr "Uptodown variant selected: id=${best_id}, stage=${best_stage}, dpi-distance=${best_distance}, size=${best_size} bytes"
		# IMPORTANT: the selected variant ID is resolved through Uptodown's
		# variant download page.  For the current Uptodown web flow, the canonical
		# URL is /android/download/<id>-x.  That page contains the real data-url
		# used by the dw.uptodown.com CDN.  Do not construct the CDN URL directly
		# from data-file-id.  Keep the older endpoints only as fallbacks.
		__UPTODOWN_DIRECT_URL__=""
		local selected_data_url="" selected_path resp_try
		local selected_paths=("/download/${best_id}-x" "/download/${best_id}" "/post-download/${best_id}")
		for selected_path in "${selected_paths[@]}"; do
			selected_download_url="${uptodown_dlurl}${selected_path}"
			resp_try=""
			if _cf_get "$selected_download_url"; then
				resp_try="$html"
			else
				resp_try=$(req "$selected_download_url" - 2>/dev/null) || continue
			fi

			# Current Uptodown download pages use data-button-id rather than an id
			# on the download button. Prefer that exact element first.
			selected_data_url=$($HTMLQ '[data-button-id="detail-download-button"]' --attribute data-url <<<"$resp_try" 2>/dev/null || true)
			if [ -z "$selected_data_url" ]; then
				selected_data_url=$($HTMLQ "#detail-download-button" --attribute data-url <<<"$resp_try" 2>/dev/null || true)
			fi
			if [ -z "$selected_data_url" ]; then
				selected_data_url=$($HTMLQ '[data-button-id="detail-download-button"]' --attribute href <<<"$resp_try" 2>/dev/null || true)
			fi
			if [ -z "$selected_data_url" ]; then
				selected_data_url=$($HTMLQ "#detail-download-button" --attribute href <<<"$resp_try" 2>/dev/null || true)
			fi
			# IMPORTANT: do NOT grab the first generic data-url on the page.
			# Uptodown pages contain unrelated data-url attributes (e.g. "apps"),
			# which would incorrectly produce https://dw.uptodown.com/dwn/apps.
			# Only accept the download button/post-download URL or a real CDN URL.
			if [ -z "$selected_data_url" ]; then
				selected_data_url=$(sed -n 's/.*data-button-id="detail-download-button"[^>]*data-url="\([^"]*\)".*/\1/p' <<<"$resp_try" | head -1 || true)
			fi
			if [ -z "$selected_data_url" ]; then
				selected_data_url=$(grep -oE 'data-url="[^"]{20,}"' <<<"$resp_try" | head -1 | sed 's/^data-url="//;s/"$//' || true)
			fi
			if [ -z "$selected_data_url" ]; then
				selected_data_url=$(sed -n 's/.*class="post-download"[^>]*data-url="\([^"]*\)".*/\1/p' <<<"$resp_try" | head -1 || true)
			fi
			if [ -z "$selected_data_url" ]; then
				selected_data_url=$(grep -oE 'https://dw[0-9]*\.uptodown\.(com|net)/dwn/[A-Za-z0-9_./+=?&%-]+' <<<"$resp_try" | head -1 || true)
			fi
			# Never accept generic site values such as data-url="apps".
			if [ "$selected_data_url" = "apps" ] || [ "$selected_data_url" = "" ]; then
				selected_data_url=""
			fi
			if [ -n "$selected_data_url" ]; then
				break
			fi
		done

		[ -n "$selected_data_url" ] || return 1
		if [[ "$selected_data_url" == https://* ]]; then
			__UPTODOWN_DIRECT_URL__="$selected_data_url"
		else
			__UPTODOWN_DIRECT_URL__="https://dw.uptodown.net/dwn/${selected_data_url}"
		fi
	fi

	local data_url direct_url="${__UPTODOWN_DIRECT_URL__:-}"
	if [ -z "$direct_url" ]; then
		data_url=$($HTMLQ "#detail-download-button" --attribute data-url <<<"$resp" 2>/dev/null || true)
		if [ -z "$data_url" ]; then
			data_url=$($HTMLQ "#detail-download-button" --attribute href <<<"$resp" 2>/dev/null || true)
		fi
		[ -n "$data_url" ] || return 1
		if [[ "$data_url" == https://* ]]; then
			direct_url="$data_url"
		else
			direct_url="https://dw.uptodown.net/dwn/${data_url}"
		fi
	fi
	# Uptodown's web CDN can return a Cloudflare HTML challenge to GitHub Actions
	# even when the browser page is accessible. If the downloaded object is not
	# actually an APK/XAPK, use Uptodown's authenticated eAPI to obtain the
	# signed CDN URL for the exact selected file ID.
	_uptodown_api_key() {
		local secret='$(=a%·!45J&S'
		local hour_epoch=$(( $(date +%s) / 3600 * 3600 ))
		printf '%s' "${secret}${hour_epoch}" | sha256sum | awk '{print $1}'
	}
	_uptodown_api_app_id() {
		local pkg api_key api_resp
		pkg=$($HTMLQ --text "tr.full:nth-child(1) > td:nth-child(3)" <<<"$__UPTODOWN_RESP_PKG__" 2>/dev/null || true)
		[ -n "$pkg" ] || pkg=$($HTMLQ --text "tr.full:nth-child(1) > td:nth-child(3)" <<<"$__UPTODOWN_RESP__" 2>/dev/null || true)
		[ -n "$pkg" ] || return 1
		api_key=$(_uptodown_api_key)
		api_resp=$(curl -fsSL --retry 2 --connect-timeout 15 --max-time 30 \
			-H 'User-Agent: Dalvik/2.1.0 (Linux; U; Android 14; SM-G955F Build/AP2A.240805.005)' \
			-H 'Identificador: Uptodown_Android' \
			-H 'Identificador-Version: 707' \
			-H "APIKEY: ${api_key}" \
			"https://www.uptodown.app/eapi/apps/byPackagename/${pkg}" 2>/dev/null) || return 1
		jq -er '.data.appID // .data.id // .appID // .id' <<<"$api_resp" 2>/dev/null
	}
	_uptodown_api_direct_url() {
		local file_id="$1" app_id api_key api_resp
		[ -n "$file_id" ] || return 1
		app_id=$(_uptodown_api_app_id) || return 1
		api_key=$(_uptodown_api_key)
		api_resp=$(curl -fsSL --retry 2 --connect-timeout 15 --max-time 30 \
			-H 'User-Agent: Dalvik/2.1.0 (Linux; U; Android 14; SM-G955F Build/AP2A.240805.005)' \
			-H 'Identificador: Uptodown_Android' \
			-H 'Identificador-Version: 707' \
			-H "APIKEY: ${api_key}" \
			"https://www.uptodown.app/eapi/apps/${app_id}/file/${file_id}/downloadUrl?update=0" 2>/dev/null) || return 1
		jq -er '.data.downloadURL' <<<"$api_resp" 2>/dev/null
	}
	_uptodown_curl_download() {
		local url="$1" out="$2" referer="${3:-}"
		local ua='Dalvik/2.1.0 (Linux; U; Android 14; SM-G955F Build/AP2A.240805.005)'
		rm -f "$out"
		if [ -n "$referer" ]; then
			curl -fL --retry 3 --retry-all-errors --connect-timeout 20 --max-time 900 -A "$ua" -H 'Accept: */*' -e "$referer" -o "$out" "$url"
		else
			curl -fL --retry 3 --retry-all-errors --connect-timeout 20 --max-time 900 -A "$ua" -H 'Accept: */*' -o "$out" "$url"
		fi
	}
	_uptodown_valid_archive() {
		local f="$1"
		[ -s "$f" ] || return 1
		# APK/XAPK/APKM are ZIP containers. Reject HTML/Cloudflare pages.
...
PY
	...
	if [ "$is_bundle" = true ]; then
		rm -f "$output.apkm"
		if _uptodown_curl_download "$direct_url" "$output.apkm" "$selected_download_url" >/dev/null 2>&1 && _uptodown_valid_archive "$output.apkm"; then
			merge_splits "${output%.apk}.apkm" "${output}"
		else
			local api_direct
			api_direct=$(_uptodown_api_direct_url "$best_id" 2>/dev/null || true)
			[ -n "$api_direct" ] || return 1
			rm -f "$output.apkm"
			_uptodown_curl_download "$api_direct" "$output.apkm" "$selected_download_url" >/dev/null 2>&1 || return 1
			_uptodown_valid_archive "$output.apkm" || return 1
			merge_splits "${output%.apk}.apkm" "${output}"
		fi
	else
		rm -f "$output"
		if _uptodown_curl_download "$direct_url" "$output" "$selected_download_url" >/dev/null 2>&1 && _uptodown_valid_archive "$output"; then
			return 0
		fi
		local api_direct
		api_direct=$(_uptodown_api_direct_url "$best_id" 2>/dev/null || true)
		[ -n "$api_direct" ] || { pr "ERROR: Uptodown web download returned a non-APK response and eAPI fallback failed"; return 1; }
		rm -f "$output"
		_uptodown_curl_download "$api_direct" "$output" "$selected_download_url" >/dev/null 2>&1 || return 1
		_uptodown_valid_archive "$output" || { pr "ERROR: Uptodown eAPI returned a non-APK response"; return 1; }
	fi
}
...
# Validate an APK/APKM already restored from GitHub Actions cache before using it.
# A cache hit is not proof that the file is usable: a previous failed Uptodown
# request can leave an HTML/Cloudflare response behind.  Require a real ZIP/APK,
# a root AndroidManifest.xml for plain APKs, and (when available) the expected
# package/version. Signature is checked only when this project has a known entry
# for the package in sig.txt.
validate_cached_download() {
	local file="$1" pkg_name="$2" expected_version="${3:-}"
	local target="$file"
	local temp_dir=""
	local aapt_cmd=""
	local package_name="" version_name=""

	if [ -f "${file%.apk}.apkm" ]; then
		target="${file%.apk}.apkm"
	fi
	[ -s "$target" ] || return 1
	...
	# APKM/APKS/XAPK containers are validated structurally here; their base APK
	# will be checked by verify_downloaded_apk immediately before patching.
	if [ "$target" != "$file" ]; then
		if ! unzip -l "$target" 2>/dev/null | grep -Eq '(^|[[:space:]])(base|[^/]+)\.apk$'; then
			return 1
		fi
		return 0
	fi

	if ! unzip -l "$file" 2>/dev/null | grep -q 'AndroidManifest\.xml$'; then
		return 1
	fi

	if command -v aapt >/dev/null 2>&1; then
		aapt_cmd="$(command -v aapt)"
	elif [ -n "${ANDROID_SDK_ROOT:-}" ]; then
		aapt_cmd="$(find "$ANDROID_SDK_ROOT/build-tools" -type f -name aapt -perm -111 2>/dev/null | sort -V | tail -1 || true)"
	fi
	if [ -n "$aapt_cmd" ] && [ -x "$aapt_cmd" ]; then
		package_name=$("$aapt_cmd" dump badging "$file" 2>/dev/null | grep -oP "package: name='\\K[^']+" | head -1 || true)
		version_name=$("$aapt_cmd" dump badging "$file" 2>/dev/null | grep -oP "versionName='\\K[^']+" | head -1 || true)
		[ -n "$package_name" ] || return 1
		[ -z "$pkg_name" ] || [ "$package_name" = "$pkg_name" ] || return 1
		[ -z "$expected_version" ] || [ -z "$version_name" ] || [ "$version_name" = "$expected_version" ] || return 1
	fi

	# If sig.txt contains this package, also verify its known certificate.
	if grep -q "$pkg_name" sig.txt 2>/dev/null; then
		check_sig "$file" "$pkg_name" >/dev/null 2>&1 || return 1
	fi
	return 0
}
...
build_rv() {
	...
	for arch in "${arch_list[@]}"; do
		...
		# Never trust a restored cache entry merely because the path exists.
		# Invalid/stale files are removed so the downloader is forced to fetch a
		# fresh copy instead of passing HTML/corrupt ZIP data to Morphe.
		if [ -f "$stock_apk" ] && ! validate_cached_download "$stock_apk" "$pkg_name" "$version"; then
			epr "Cached APK is invalid for ${pkg_name} v${version}; removing and re-downloading."
