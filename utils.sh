...
dl_uptodown() {
	...
	if [ -n "$data_version" ]; then
		...
		# Uptodown's current download endpoint uses the raw file ID. Older builds of
		# this parser appended "-x", which can return a page without data-url and
		# ultimately produce https://dw.uptodown.com/dwn/ (404). Keep -x only as
		# a compatibility fallback if the raw-ID endpoint does not expose data-url.
		resp=$(req "${uptodown_dlurl}/download/${best_id}" -) || return 1
		local selected_data_url
		selected_data_url=$($HTMLQ "#detail-download-button" --attribute data-url <<<"$resp" 2>/dev/null || true)
		if [ -z "$selected_data_url" ]; then
			resp=$(req "${uptodown_dlurl}/download/${best_id}-x" -) || return 1
		fi
	fi
