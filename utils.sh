...
dl_uptodown() {
	...
	if [ -n "$data_version" ]; then
		...
		for ((n = 1; n <= max_children; n++)); do
			...
			candidates+=("${data_file_id}|${file_type}|${node_arch}|${node_dpi}|${size_bytes}")
		done
		...
		local best_id="" best_type="" best_size=999999999999999 best_distance=999999 found=false c
		for c in "${candidates[@]}"; do
			local cid ct ca cd csz
			IFS='|' read -r cid ct ca cd csz <<<"$c"
