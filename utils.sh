...
apkmirror_search() {
	...
	# Generic APKMirror selection policy:
	# 1) arm64-v8a + DPI/range reaching 522 or higher -> smallest file size.
	# 2) if none: arm64-v8a / universal / noarch + nodpi -> smallest file size.
	# 3) if none: arm64-v8a / universal / noarch -> closest DPI to 522.
	#    File size is NOT used in stage 3.
	# APK and Bundle APK are treated identically.
	...
	# APKMirror shows the exact size in the release/variant row, for example:
	#   124.31 MB (130,350,399 bytes)
	# Always use the exact "(N bytes)" value when available.
	row_size_bytes() {
		local row="$1" bytes="" text="" s=""

		# Search the raw row first. This avoids accidentally reading a size
		# belonging to another cell/variant.
		bytes=$(grep -oP '\([[:space:]]*[0-9][0-9,]*[[:space:]]+bytes[[:space:]]*\)' <<<"$row" |
			head -1 |
			grep -oP '[0-9][0-9,]*' |
			tr -d ',') || true
		...
		# Fallback for APKMirror markup where the text is exposed only after
		# HTML parsing. This fallback is only used when the exact byte count
		# is absent.
		text=$($HTMLQ "div.table-cell" --text <<<"$row" 2>/dev/null |
			tr '\r\n' ' ' | tr -s ' ') || true

		bytes=$(grep -oP '\([[:space:]]*[0-9][0-9,]*[[:space:]]+bytes[[:space:]]*\)' <<<"$text" |
			head -1 |
			grep -oP '[0-9][0-9,]*' |
			tr -d ',') || true
		...
		s=$(grep -oP '[0-9]+([.][0-9]+)?[[:space:]]*(KB|MB|GB)' <<<"$text" |
			tail -1) || true
	...
	# Candidate format:
	# url|type|arch|dpi|stage|distance|size
	local candidates=()
	...
	for ((n = 1; n < 80; n++)); do
		...
		local node_apk_bundle node_arch node_dpi kind lo hi stage distance size
		...
		case "$kind" in
			nodpi)
				# Stage 2 is nodpi only.
				stage=2
				;;
			single:*)
				lo="${kind#single:}"
				if (( lo >= target_dpi )) && [ "$node_arch" = "arm64-v8a" ]; then
					stage=1
				else
			...
			range:*)
				...
				# Any arm64-v8a range reaching 522 or higher satisfies stage 1.
				if (( hi >= target_dpi )) && [ "$node_arch" = "arm64-v8a" ]; then
					stage=1
					...
					# Stage 3: closest supported DPI/range to 522.
		...
		size=$(row_size_bytes "$node")

		candidates+=("${dlurl}|${node_apk_bundle}|${node_arch}|${node_dpi}|${stage}|${distance}|${size}")
	done

	[ ${#candidates[@]} -eq 0 ] && return 1

	# First select the highest-priority stage.
	...
	local best_url="" best_type="" best_arch="" best_dpi=""
	local best_size=999999999999999
	...
	for c in "${candidates[@]}"; do
		...
		if [ "$best_stage" -eq 1 ] || [ "$best_stage" -eq 2 ]; then
			# Stages 1 and 2: smallest file size wins.
			...
			# Stage 3: closest DPI wins. File size is deliberately ignored.
	...
	pr "APKMirror variant selected: arch=${best_arch}, dpi=${best_dpi}, stage=${best_stage}, size=${best_size} bytes"
