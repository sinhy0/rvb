...
_unqueued_cf_get() {
	# Prefer the normal HTTP path first.  This avoids depending on the
	# currently deployed Trawl version for pages that are directly reachable.
	# If Cloudflare blocks the direct request, fall back to Trawl when enabled.
	if _fallback_get "$@"; then
		return 0
	fi
