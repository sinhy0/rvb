...
_trawl_8191_get() {
	...
	for attempt in $(seq 1 $max_retries); do
		...
		response=$(curl -m 90 -s -X POST "$solver_url" \
			...
			-d "{\"url\":\"$url\",\"maxTimeout\":60000,\"skipHttp\":false${extra_headers}}") || true
...
_fallback_get(){
	local url=$1
	html=$(curl -L -c "$TEMP_DIR/cookie.txt" -b "$TEMP_DIR/cookie.txt" --connect-timeout 10 --retry 1 -s -f "$url" \
        -H "User-Agent: Mozilla/5.0 (X11; Linux x86_64; rv:109.0) Gecko/20100101 Firefox/109.0" \
        -H "Accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,*/*;q=0.8" \
        -H "Accept-Language: en-US,en;q=0.5" \
        -H "Upgrade-Insecure-Requests: 1") || return 1
...
_unqueued_cf_get() {
	# Trawl is an optional first attempt. If it cannot solve the page,
	# always fall back to a normal browser-like request instead of failing
	# immediately. This is important for APKMirror pages that Trawl may reject
	# even though a direct request is still available.
