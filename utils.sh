...
apkmirror_search() {
	...
	row_size_bytes() {
		...
		# APKMirror exposes the exact file size as:
		#   124.31 MB (130,350,399 bytes)
		# Always prefer the exact byte count in parentheses.
		bytes=$(grep -oP '\([[:space:]]*[0-9][0-9,]*[[:space:]]+bytes[[:space:]]*\)' <<<"$text" |
			head -1 | grep -oP '[0-9][0-9,]*' | tr -d ',') || true
		...
		# Fallback only when APKMirror does not expose the byte count.
