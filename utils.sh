...
_get_patch_last_supported_ver() {
	...
	# Always choose the NEWEST app version supported by the current patches.
	# Do NOT use Morphe/ReVanced's "Most common compatible versions" ranking
	# (which can prefer an older version with the same/larger patch count).
	#
	# Example:
	#   21.35.442 (80 patches)
	#   21.34.243 (80 patches)
	#   20.51.39  (80 patches)
	# -> select 21.35.442.
	#
	# Keep every version reported by the patch CLI and let get_highest_ver()
	# perform semantic-version ordering. This applies to every app using
	# version=auto, not just a specific package.
	local supported_versions=""
	while IFS= read -r line; do
		[ -z "$line" ] && continue
		# The CLI normally prints: VERSION (N patches) [optional metadata]
		local v
		v=$(sed -E 's/^v?([^[:space:]]+).*/\1/' <<<"$line")
		if [ -n "$v" ] && semver_validate "$v"; then
	...
	if [ -n "$supported_versions" ]; then
		get_highest_ver <<<"$supported_versions" || return 1
	fi
}
...
apkmirror_search() {
	...
	# 1) arm64-v8a + DPI that supports 522 (a single DPI >=522, or a range containing 522)
	#    -> smallest file size. APK and BUNDLE compete together.
	# 2) If none: nodpi + (arm64-v8a / universal / noarch)
	#    -> smallest file size. APK and BUNDLE compete together.
	# 3) If none: closest DPI to 522 + (arm64-v8a / universal / noarch)
	#    -> closest DPI, regardless of size. APK and BUNDLE compete together.
	#
	# No other architecture is allowed in any stage.

	# Return 0 when a DPI value/range satisfies stage 1.
	dpi_supports_target() {
		local value="${1,,}"
		value="$(xargs <<<"$value")"
		if [[ "$value" =~ ^([0-9]+)$ ]]; then
			(( BASH_REMATCH[1] >= target_dpi ))
			return
		fi
		if [[ "$value" =~ ^([0-9]+)-([0-9]+)$ ]]; then
			local lo=${BASH_REMATCH[1]} hi=${BASH_REMATCH[2]}
			(( lo <= target_dpi && target_dpi <= hi ))
			return
		fi
		return 1
	}

	# Return the distance from 522 to a numeric DPI/range.
	dpi_distance() {
		local value="${1,,}"
		value="$(xargs <<<"$value")"
		local lo hi
		...
		if (( target_dpi < lo )); then
			echo $((lo - target_dpi))
		elif (( target_dpi > hi )); then
			echo $((target_dpi - hi))
		else
			echo 0
		fi
	...
	# Get the actual file size from the selected variant's APKMirror detail page.
	# The first "NN MB (NN bytes)" occurrence is the current variant's size.
	get_variant_size_bytes() {
		local variant_url="$1" page size_bytes
		__SILENT_CF_GET__=true _cf_get "$variant_url" || return 1
		page="$html"
		size_bytes=$(printf '%s' "$page" | grep -oP '\([0-9][0-9,]*\s*bytes\)' | head -1 | grep -oP '[0-9][0-9,]*' | tr -d ',') || true
	...
	# Candidate records: URL<TAB>ARCH<TAB>DPI<TAB>TYPE
	local -a stage1=() stage2=() stage3=()
	for ((n = 1; n < 40; n++)); do
		...
		node_apk_bundle="${node_apk_bundle^^}"
		...
		node_dpi="${node_dpi//–/-}"
		node_dpi="${node_dpi//—/-}"
		node_dpi="$(xargs <<<"$node_dpi")"
		if [ "$node_dpi" != "nodpi" ] && [ "$node_dpi" != "anydpi" ]; then
			node_dpi="${node_dpi//dpi/}"
			node_dpi="$(xargs <<<"$node_dpi")"
		fi

		# ANY means APK and BUNDLE are intentionally considered together.
		if [ "$apk_bundle" != "ANY" ] && [ "$node_apk_bundle" != "${apk_bundle^^}" ]; then
			continue
		fi
		...
		if [ "$node_arch" = "arm64-v8a" ] && dpi_supports_target "$node_dpi"; then
			stage1+=("$dlurl"$'\t'"$node_arch"$'\t'"$node_dpi"$'\t'"$node_apk_bundle")
		elif [ "$node_dpi" = "nodpi" ] && is_allowed_fallback_arch "$node_arch"; then
			stage2+=("$dlurl"$'\t'"$node_arch"$'\t'"$node_dpi"$'\t'"$node_apk_bundle")
		elif is_allowed_fallback_arch "$node_arch" && [ "$node_dpi" != "nodpi" ]; then
			stage3+=("$dlurl"$'\t'"$node_arch"$'\t'"$node_dpi"$'\t'"$node_apk_bundle")
		fi
	done

	local best_url="" best_size=-1 best_distance=999999999 best_arch_rank=999999999
	local rec rurl rarch rdpi rtype rsize rdist

	# Stage 1: smallest actual file size.
	if ((${#stage1[@]} > 0)); then
		for rec in "${stage1[@]}"; do
			IFS=$'\t' read -r rurl rarch rdpi rtype <<<"$rec"
			rsize=$(get_variant_size_bytes "$rurl") || continue
			if (( best_size < 0 || rsize < best_size )); then
				best_size=$rsize
				best_url=$rurl
			fi
		done
	...
	# Stage 2: smallest actual file size among allowed architectures with nodpi.
	best_size=-1
	best_url=""
	if ((${#stage2[@]} > 0)); then
		for rec in "${stage2[@]}"; do
			IFS=$'\t' read -r rurl rarch rdpi rtype <<<"$rec"
			rsize=$(get_variant_size_bytes "$rurl") || continue
			if (( best_size < 0 || rsize < best_size )); then
				best_size=$rsize
	...
	# Stage 3: closest DPI to 522, regardless of size.
	best_distance=999999999
	best_arch_rank=999999999
	best_url=""
	if ((${#stage3[@]} > 0)); then
		for rec in "${stage3[@]}"; do
			IFS=$'\t' read -r rurl rarch rdpi rtype <<<"$rec"
			rdist=$(dpi_distance "$rdpi")
			case "$rarch" in
				arm64-v8a) rarch_rank=0 ;;
				universal) rarch_rank=1 ;;
				noarch) rarch_rank=2 ;;
				*) rarch_rank=9 ;;
			esac
			# If DPI distance ties, keep a deterministic architecture preference.
			if (( rdist < best_distance || (rdist == best_distance && rarch_rank < best_arch_rank) )); then
				best_distance=$rdist
				best_arch_rank=$rarch_rank
				best_url=$rurl
			fi
		done
	fi
...
dl_apkmirror() {
	...
	if [ "$node" ]; then
		# APK and BUNDLE are intentionally evaluated together by the selection policy.
		if ! dlurl=$(apkmirror_search "$resp" "$dpi" "$arch" "ANY" "$clean_search_version" "$search_version"); then
			return 1
		fi

		_cf_get "$dlurl" || return 1
		resp="$html"
		# Determine whether the selected variant is a Bundle from its detail page.
		if grep -qiE 'Download APK Bundle|APK Bundle Base APK|Download APK Bundle Base APK' <<<"$resp"; then
			is_bundle=true
		else
			is_bundle=false
		fi
