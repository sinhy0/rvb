...
source_release_pick_from_list() {
	local host=${1,,}
	# Always select the newest release by publication/release timestamp.
	# The release channel (stable/dev/alpha/beta/rc) is intentionally NOT
	# used for ordering or filtering.  This makes "latest" truly mean the
	# most recently published release, regardless of channel.
	case "$host" in
		github)
...
_get_prebuilts() {
	...
	if [ "$ver" = "dev" ] || [ "$ver" = "latest" ]; then
		...
		release=$(source_release_pick_from_list "$host" latest <<<"$resp") || true
...
apkmirror_search() {
	...
	# Generic APKMirror variant policy:
	#   1) arm64-v8a variants with DPI >= 522 -> smallest file
	#   2) nodpi/anydpi among arm64-v8a/universal/noarch -> smallest file
	#   3) otherwise the closest numeric DPI to 522 among arm64-v8a/universal/noarch
	# APK and BUNDLE are evaluated together; row order never decides the winner.

	dpi_kind() {
		local value="${1,,}"
		value="$(xargs <<<"$value")"
		value="${value//dpi/}"
		if [[ "$value" == "nodpi" || "$value" == "any" ]]; then
			echo "special"
			return 0
		fi
		if [[ "$value" =~ ^([0-9]+)-([0-9]+)$ ]]; then
			...
			echo "range:${BASH_REMATCH[1]}:${BASH_REMATCH[1]}"
		else
			echo "unknown"
		fi
	}
	...
	arch_allowed() {
		local a="${1,,}"
		case "$a" in
			arm64-v8a|universal|noarch) return 0 ;;
			*) return 1 ;;
		esac
	}

	# Convert a displayed file size to bytes. Unknown/missing size sorts last.
	size_bytes() {
		local v="${1//,/}"
		local n u
		n=$(grep -oP '[0-9]+([.][0-9]+)?' <<<"$v" | head -1) || true
		u=$(grep -oP '(?i)(KB|MB|GB|B)' <<<"$v" | head -1) || true
		[ -z "$n" ] && { echo 999999999999999; return; }
		case "${u^^}" in
			GB) awk -v n="$n" 'BEGIN {printf "%.0f", n*1024*1024*1024}' ;;
			MB) awk -v n="$n" 'BEGIN {printf "%.0f", n*1024*1024}' ;;
	...
	variant_size_bytes() {
		local variant_url="$1" page="" plain="" size_text=""
	...
	# Candidate arrays: url|type|arch|dpi|stage|dpi_distance|size
	local candidates=()
	for ((n = 1; n < 80; n++)); do
		...
		local node_apk_bundle node_arch node_dpi kind lo hi stage distance
		node_apk_bundle=$($HTMLQ \
			"div.table-cell:nth-child(1) span.apkm-badge:first-of-type" \
			--text <<<"$node" 2>/dev/null | xargs || true)
		...
		arch_allowed "$node_arch" || continue

		kind=$(dpi_kind "$node_dpi")
		stage=3
		distance=999999
		...
		if [ "$kind" = "special" ]; then
			...
			IFS=: read -r _ lo hi <<<"$kind"
			# Preferred variant: arm64-v8a with DPI/range reaching 522 or higher.
			# A DPI range qualifies when its upper bound is >= 522, because
			# the range contains at least one supported density >= 522.
			if [ "$node_arch" = "arm64-v8a" ] && (( hi >= target_dpi )); then
				stage=1
				distance=0
			elif (( lo <= target_dpi && target_dpi <= hi )); then
				distance=0
			elif (( lo > target_dpi )); then
				distance=$((lo-target_dpi))
			else
				distance=$((target_dpi-hi))
			fi
		else
			continue
		fi

		candidates+=("${dlurl}|${node_apk_bundle}|${node_arch}|${node_dpi}|${stage}|${distance}")
	done

	[ ${#candidates[@]} -eq 0 ] && return 1

	# Determine the winning stage first. Lower stage number has priority.
	...
	# Allowed architectures were enforced above.
	# Stages 1 and 2: smallest file wins. Stage 3: closest DPI wins,
	# with file size as the tie-breaker.
	local best_url="" best_type="" best_size=999999999999999
	local best_distance=999999 best_width=999999
	local found=false

	for c in "${candidates[@]}"; do
		local u t a d st dist sz width
		u=$(cut -d'|' -f1 <<<"$c")
		t=$(cut -d'|' -f2 <<<"$c")
		a=$(cut -d'|' -f3 <<<"$c")
		d=$(cut -d'|' -f4 <<<"$c")
		st=$(cut -d'|' -f5 <<<"$c")
		dist=$(cut -d'|' -f6 <<<"$c")
		[ "$st" -ne "$best_stage" ] && continue

		sz=$(variant_size_bytes "$u")
		width=999999
		if [[ "$d" =~ ^([0-9]+)-([0-9]+)dpi$ ]]; then
			width=$((BASH_REMATCH[2]-BASH_REMATCH[1]))
		elif [[ "$d" =~ ^([0-9]+)dpi$ ]]; then
			width=0
		fi
		...
		if [ "$best_stage" -eq 1 ] || [ "$best_stage" -eq 2 ]; then
			if [ "$sz" -lt "$best_size" ]; then
				best_url="$u"; best_type="$t"; best_size="$sz"; best_distance="$dist"; best_width="$width"; found=true
			fi
		else
			if [ "$dist" -lt "$best_distance" ] ||
			   { [ "$dist" -eq "$best_distance" ] && [ "$sz" -lt "$best_size" ]; }; then
				best_url="$u"; best_type="$t"; best_size="$sz"; best_distance="$dist"; best_width="$width"; found=true
			fi
		fi
	done
	...
	[ "$found" = true ] || return 1
	pr "APKMirror variant selected: arch=${a:-unknown}, dpi=${d:-unknown}, stage=${best_stage}, size=${best_size} bytes"
