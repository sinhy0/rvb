...
apkmirror_search() {
	...
	# Returns: stage|distance. A numeric DPI range is treated as a range,
	# so a range whose lower bound is >=522 satisfies stage 1; otherwise its
	# distance to 522 is used for stage 3.
	variant_stage_distance() {
		local a="${1,,}" d="${2,,}" raw lo hi value
		a="$(xargs <<<"$a")"; d="$(xargs <<<"$d")"; raw="${d//dpi/}"
		case "$a" in arm64-v8a|universal|noarch) ;; *) echo ""; return;; esac

		if [ "$raw" = "nodpi" ]; then echo "2|0"; return; fi

		if [[ "$raw" =~ ^([0-9]+)-([0-9]+)$ ]]; then
			lo="${BASH_REMATCH[1]}"; hi="${BASH_REMATCH[2]}"
			if [ "$a" = "arm64-v8a" ] && (( lo >= target_dpi )); then
				echo "1|$((lo-target_dpi))"; return
			fi
		...
		if [[ "$raw" =~ ^([0-9]+)$ ]]; then
			...
			if [ "$a" = "arm64-v8a" ] && (( value >= target_dpi )); then
	...
	for c in "${candidates[@]}"; do
		...
		if [ "$best_stage" -eq 1 ] || [ "$best_stage" -eq 2 ]; then
			...
			# Stage 3: ONLY DPI distance decides. Never compare size here.
			if [ "$dist" -lt "$best_distance" ]; then best_url="$u"; best_type="$t"; best_distance="$dist"; best_size="$sz"; found=true; fi
		fi
	done
	[ "$found" = true ] || return 1
